...
class PosSession(models.Model):
    ...
    def _validate_session(self):
        ...
        is_closed_before = self.state == 'closed'

        if is_closed_before:
            self = self.with_context(force_save=True)
    ...
    def _create_payment_register_invoices_payment(self):
        ...
        if invoice_payment_ids:
            ...
            for invoice in invoice_payment_ids.mapped('move_id'):
                ...
                # We need to filter for those lines that are already reconciled
                reconciled_lines = invoice_receivable_lines.filtered('reconciled')
                invoice_receivable_lines = invoice_receivable_lines - reconciled_lines

                if not invoice_receivable_lines:
                    error_msm = _(
                        "You are trying to pay an invoice (%s) that is already paid!\n Please check the following payments: (%s)") % (
                                invoice.name, ",".join(payment_ids.mapped(lambda p: p.name or '')))
                    raise UserError(error_msm)
